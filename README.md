...
Genome Evidence Audit is a static, browser-only research application for _Klebsiella_ assembled genomes. Version `0.3.1-alpha.1` places two reproducible workflows on one website:
...
Genome Evidence Audit contributors. Genome Evidence Audit browser edition. Version 0.3.1-alpha.1. 2026. <https://github.com/al-mualm/genome-evidence-audit/releases/tag/v0.3.1-alpha.1>
...
## Publication figures and citation

Completed analyses include an on-page evidence matrix and one-click downloads for editable SVG, three-times-resolution PNG, analysis-ready CSV and a suggested figure caption. SVG is the preferred journal format because labels and shapes remain editable and resolution-independent. Researchers can replace filenames with short, de-identified figure labels before analysis.

Until a peer-reviewed software article is published, the website displays a version-specific software citation and provides a BibTeX download. The notice explicitly states that the article citation will be added after publication; it does not imply that the manuscript has already been accepted.
